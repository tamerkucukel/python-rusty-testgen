...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ast_func_defs = match ast_loader::load_ast_from_file(python_file_path_str) {
        ...
        Err(e) => {
            verbose_eprintln!(
                quiet_mode,
                "Error loading AST: {}. Invalid file structure or Python syntax error.",
                e
            );
            return Err(e.into());
        }
    ...
    for func_def in &ast_func_defs {
        ...
        verbose_println!(
            quiet_mode,
            "Step 2: Building Control Flow Graph (CFG) for {}...",
            func_name
        );
        ...
        verbose_println!(
            quiet_mode,
            "Step 3: Scraping paths from CFG for {}...",
            func_name
        );
        if let Some(paths) = path::PathScraper::get_paths(&cfg_data) {
            if paths.is_empty() {
                verbose_println!(
                    quiet_mode,
                    "No executable paths found for function {}.",
                    func_name
                );
                continue;
            }
            ...
            verbose_println!(
                quiet_mode,
                "Step 4: Analyzing paths and generating Z3 constraints for {}...",
                func_name
            );
            ...
            if !quiet_mode {
                verbose_println!(
                    quiet_mode,
                    "\nConstraint Analysis Summary for {}:",
                    func_name
                );
                for result in &path_constraint_results {
                    if result.is_satisfiable {
                        verbose_println!(
                            quiet_mode,
                            "  Path {}: ✅ Satisfiable. Model: {}",
                            result.path_index,
                            result.model.as_deref().unwrap_or("N/A")
                        );
                    } else {
                        verbose_println!(
                            quiet_mode,
                            "  Path {}: ❌ Unsatisfiable. Error: {}",
                            result.path_index,
                            result
                                .error
                                .as_ref()
            ...
            verbose_println!(
                quiet_mode,
                "Step 5: Generating Pytest code for {}...",
                func_name
            );
            ...
            if !pytest_string.contains("def test_") {
                // A simple check if any test function was actually generated
                verbose_println!(
                    quiet_mode,
                ...
                let test_file_name =
                    format!("test_{}.py", func_name.to_lowercase().replace(" ", "_"));
                verbose_println!(
                    quiet_mode,
                    "Attempting to write tests to {}...",
                    test_file_name
                );
                match fs::write(&test_file_name, pytest_string) {
                    Ok(_) => {
                        verbose_println!(
                            quiet_mode,
                            "Successfully generated and wrote tests to {}",
                            test_file_name
                        );
            ...
            verbose_println!(
                quiet_mode,
                "No paths found by PathScraper for function {}.",
                func_name
            );
        }
    }
    ...
    if !tests_generated_overall && ast_func_defs.is_empty() {
        verbose_println!(
            quiet_mode,
            "No tests were generated because no functions were found or processed."
        );
    } else if !tests_generated_overall {
        verbose_println!(
            quiet_mode,
            "No tests were generated for any function (e.g. no satisfiable paths or other issues)."
        );
    }
